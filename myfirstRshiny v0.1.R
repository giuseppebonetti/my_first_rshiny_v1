    ...
    fluidRow(
      ...
      tags$table(
        ...
        tags$tbody(
          tags$tr(
            tags$td("IA utility"),
            tags$td(numericInput("utility_ia", label = NULL, value = 0.59, width = "100%"))
          ),
          tags$tr(
            tags$td("Success utility"),
            tags$td(numericInput("utility_success", label = NULL, value = 0.63, width = "100%"))
          ),
          tags$tr(
            tags$td("Failure utility"),
            tags$td(numericInput("utility_failure", label = NULL, value = 0.59, width = "100%"))
          ),
          tags$tr(
...
server <- function(input, output){
  ...
  observeEvent(input$run_model, {
    ...
    sum_result <- input$drugcost_olorofim + input$drugcost_baat + input$slider + 
      as.numeric(input$timehorizon) + input$cost_icu + input$cost_ward + 
      input$cost_ivathome + input$rate_mortality + input$utility_success + 
      input$utility_failure + input$utility_ia + input$disutility_ivathome + 
      value_from_excel
